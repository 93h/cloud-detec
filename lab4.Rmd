...
```{r label_proportion, fig.cap="Proportion of Labels in Each Image", fig.height=3}
## summarizing data
...
load('other/cvCompare.RData')
knitr::kable(df, caption = 'Cross-Validation Comparison')
```

## Post-hoc EDA for random forest
...
certain_images <- images %>%
  filter(label != 0) %>%
  mutate(label = as.factor(label))
certain_images$label <- factor(certain_images$label)

rf <- randomForest(label ~ NDAI + SD + CORR + AN, data = certain_images, ntree = 100)

#get predicted classes
preds <- predict(rf, data = certain_images)

#combine with the dataset
pred_images <- cbind(certain_images, as.matrix(preds))
colnames(pred_images)[15] <- "predicted_labels"

# detect misclassification
pred_images <- pred_images %>%
  mutate(label = as.integer(label)) %>%
  mutate(predicted_labels = as.integer(predicted_labels)) %>%
